...
The stage has had collisions since cycle 2, but it's been hard to stop players from passing through it if they collide with the corners. I attempted to fix the collision in cycle 3, cycle 5 and cycle 8 due to the advanced movement options like double jumps, dashes and being attached to specific moves
...
Moves and lives were made in cycle 4, but storing their values in cycle 7 proved tough as I had to pass a value from game.cpp to character.cpp and then to move.cpp. I eventually initialised a vector in game.h with each character's stats and moves being stored in each parameter and all the values being individual values.

### Criterion 7 & 8

I implemented invincibility in cycle 7. If a player hit an opponent with invincibility, they would be in the normal end lag for the move but the opponent would be unaffected. After that, I added an invincible dodge and a cooldown for it. However, I wanted to make characters' dodge last for different lengths as well as the cooldowns without adding a new character stat. I decided that invincibility frames when dodging would be 20 for all characters, but the frames they would be in end lag for was 30 \* weight and the cooldown before you could use it again would be 120 \* weight.&#x20;

### Criterion 9 & 10

I implemented these with the main menu in cycle 6, but I initially didn't reset all values when the game ended. I need to clear all moves, coordinates, the timer and other character stats like invincibility.
...
I've explained my reasoning for these specific controls in the [features](../analysis/1.4a-features-of-the-proposed-solution.md#player-inputs), but I struggled to implement a few. Dodging with player 1 uses left shift, but if I used VK\_SHIFT (0x10) then it could be triggered with right shift. This meant I had to use VK\_LSHIFT (x0A0). In addition to that, on one keyboard the game was tested on, VK\_OEM\_1(0xBA) or ";" didn't register, so player 2 couldn't use special moves. I also decided to modify the left and right inputs to allow players to easily change direction, such that if they're holding left and press right, they will immediately start moving right and the other way around.
...
Even if I prefer the look of Cycle 4's to Cycle 12, users still said the stage was easy to see.
...
As I don't trust my AI to not reach a stalemate if it goes against another AI, I've only allowed the second player to be replaced with an AI. Apart from that, the start menu works well but you have to use keyboard controls.

### Criterion 38

The HUD shows the exact percent of both players at all times by calling an array of numbers for each number in the playepercent.
...
When I introduced sounds I added music to the start menu, credits and battle. I also introduced a sound effect that changed pitch (before cycle 11) / speed (in cycle 12) depending on how much damage the hit dealt.

### Criterion 41

Due to the way I introduced inputs, you could input an action and it will perform the action (provided you're still inputting it) as soon as it can
