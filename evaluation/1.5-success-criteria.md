...
Moves and lives were made in cycle 4, but storing their values in cycle 7 proved tough as I had to pass a value from game.cpp to character.cpp and then to move.cpp. I eventually initialised a vector with each character being stored in each parameter and all the values being individual integers.
...
I've explained my reasoning for these specific controls in the features, but I struggled to implement a few. Dodging with player 1 uses left shift, but if I used VK\_SHIFT (0x10) then it could be triggered with right shift. This meant I had to use `VK_LSHIFT (0xA0)`
