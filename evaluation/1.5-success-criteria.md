...
The stage has had collisions since cycle 2, but it's been hard to stop players from passing through it if they collide with the corners. I attempted to fix the collision in cycle 3, cycle 5 and cycle 8 due to the advanced movement options like double jumps, dashes and being attached to specific moves. However, it's harder to tell that the stage is a stage after cycle 10 due to the changes in how the stage is displayed due to the DrawRect function. While it looks more natural due to this change, it appears worse.
...
Moves and lives were made in cycle 4, but storing their values in cycle 7 proved tough as I had to pass a value from game.cpp to character.cpp and then to move.cpp. I eventually initialised a vector in game.h with each character's stats and moves being stored in each parameter and all the values being individual values. However, using a vector restricted my parameters as I had to pass in 310 parameters from Game to Character, which made the code unsustainable and difficult to edit. While comments helped with this, it was still difficult.
