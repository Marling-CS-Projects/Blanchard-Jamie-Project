...
* [ ] Add a weight variable for characters
...
| ---- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | --------- |
| 1    | Test the weight variable by passing different values and observing the knockback   | The knockback to decrease the higher the variable is                                      | Turns out I only passed forward the weight variable, I never used it for anything | Pass      |
| 2    | Test the pause menu displaying based on the percentage and lives of the characters | The winning player to be on the left and them to be together if it's even                 | As expected                                                                       |           |
| 3    | Check move functions by testing dissapearOnHit and the moves dissapearing on death | Moves to dissapear on hit if enabled and moves to disappear on death                      | As expected                                                                       |           |
| 4    | Test the values of different characters by testing up light                        | The height of up light on character 1 to be higher than up light on character 2 and so on | As expected                                                                       |           |
