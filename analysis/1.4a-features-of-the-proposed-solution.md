...
Each player will have 5 lives and spawn on opposite sides of the stage. Both player's objective is to hit their opponent off the map. This is done by hitting your opponent to deal damage and knockback to them. Knockback will be represented in % and the knockback you take is directly proportinal to your %. This is only true for scalar knockback, but moves also deal fixed knockback which doesn't increase based on %. This means the knockback equation is "total knockback=fixed knockback + scalar knockback\*percent/100". Once a player goes off the screen border they've lost a life, and after a brief period respawn at the center. Once one player s out of lives, the game ends and the other player wins. If you're knocked off the stage but live, you can try to recover using your mid air jump (or jumps depending on the character) and your upwards special.
...
Shift and N will be used for dodge
{% endhint %}

#### Pause

I'm going to add a pause menu that's bound to Esc or Enter. This will pause the state of the game, display both player's controls and have an option to restart the game or go to the character select screen.
