...
This also makes it easier for new players to tell where moves are going to point.&#x20;
...
* Lag is the amount of time you have to wait before a move starts or after a move ends. High lag decreases the viability of a move. Having high start lag means your opponent can hit you before the move starts. Having high end lag means your opponent can easily punish you if you miss.
...
Light attacks are fast attacks, generally having an active hitbox in under 10 frames. They do relatively little damage but can combo into some attacks. Some light attacks can kill, but they won't combo into other moves and will deal less damage than strong attacks.&#x20;
...
The camera will be a fixed camera that showcases the entire stage and boundary of the map. This shows players all the visual information they need.
...
<mark style="color:red;">ADD WALL JUMPS TO SUCCESS CRITERIA</mark>

{% tabs %}
{% tab title="Platform support" %}
Being a .exe file once compiled means it only has Windows support (without compatibility layers). This can be solved by people on other oprtating system using a Windows VM. This is a bad solution because it adds extra work for the player.
{% endtab %}
